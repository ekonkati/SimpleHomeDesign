...
def generate_rectangular_bar_layout(b: float, D: float, cover: float, 
    ...
    y_coords_l = linspace_points(dy_bot, dy_top, n_left)
    y_coords_r = linspace_points(dy_bot, dy_top, n_right)
    ...
    # Final unique list (handling corner duplicates)
    unique_bars_list = []
    for x, y, dia in bars_set:
        found = False
        for i, (ux, uy, udia) in enumerate(unique_bars_list):
            if abs(x - ux) < 1e-4 and abs(y - uy) < 1e-4:
                if dia > udia:
                    unique_bars_list[i] = (x, y, dia) 
                found = True
                break
        if not found:
            unique_bars_list.append((x, y, dia))

    return sorted(unique_bars_list, key=lambda item: (item[1], item[0]))
...
def load_initial_section(state):
...
def main():
    ...
    section = load_initial_section(state)
