...
def effective_length_factor(restraint: str) -> float:
    # IS 456-2000 Table 28 approximation for braced/unbraced members
...
def moment_magnifier(Pu: float, le_mm: float, fck: float, Ic: float, Cm: float = 0.85, sway: bool = False) -> float:
    ...
    # If Pu > Pcr (buckling imminent), assign a large delta
    ...
    # If sway, Delta_s is used directly
    
    # Clip delta to prevent excessively large values for small Pcr margins
...
def to_json_serializable(state: dict) -> dict:
    ...
    for key, value in state.items():
        if isinstance(value, float): safe_state[key] = round(value, 6)
             ...
             # Convert list of tuples to list of lists for serialization
...
def _generate_bar_layout(b: float, D: float, cover: float, state: dict) -> List[Tuple[float, float, float]]:
    ...
    # Ensure dx_corner does not exceed half the section dimension
    ...
    # Side Bars (Left/Right) - Exclude corners if already placed by Top/Bottom
    ...
    # Remove duplicate bars at corners, keeping the larger diameter if sizes differ
    unique_locations = {}
    for x, y, dia in bars_list:
        loc = (round(x, 4), round(y, 4))
        # Keep the bar with the max diameter at each location
...
def _uniaxial_capacity_Mu_for_Pu(b: float, D: float, bars: List[Tuple[float, float, float]], fck: float, fy: float, Pu: float, axis: str) -> float:
    ...
    def forces_and_moment(c: float):
        """Calculates total Axial Force (N) and Moment (M) about the geometric center for a given neutral axis depth c."""
        ...
        for (x_abs, y_abs, dia) in bars:
            ...
            # Stress-Strain Curve (IS 456-2000 simplification: 0.87*fy yield)
            # Epsilon_yd = 0.87 * fy / ES
    ...
    cL, cR = 0.01 * dimension, 1.50 * dimension # Search range for c (from small tension to deep compression)
    NL, ML = forces_and_moment(cL)
    NR, MR = forces_and_moment(cR)

    if target <= NL: return ML # Point A (Max tension)
    if target >= NR: return MR # Point C (Max compression)

    for _ in range(60): # 60 iterations provides high precision for most cases
        cm = 0.5 * (cL + cR)
    ...
    # Return average if convergence is slow (should not happen with 60 iterations)
...
def biaxial_utilization(b: float, D: float, bars: List[Tuple[float, float, float]], fck: float, fy: float, Pu: float, Mux_eff: float, Muy_eff: float, alpha: float) -> Tuple[float, float, float]:
    ...
    # 1. Find uniaxial capacities for the given Pu
    ...
    # Ensure non-zero divisor, especially for pure axial load (Pu is high, M is near zero)
...
def plotly_cross_section(b: float, D: float, cover: float, bars: List[Tuple[float, float, float]]) -> go.Figure:
    ...
    for x_geom, y_geom, dia in bars:
        bar_x.append(x_geom)
        bar_y.append(y_geom)
        bar_size.append(dia * 2) # Plotly size is diameter
...
def plot_pm_curve_plotly(b: float, D: float, bars: List[Tuple[float, float, float]], fck: float, fy: float, Pu: float, Mu_eff: float, axis: str):
    ...
    # Reuse the force calculation logic
...
def recalculate_properties(state):
    ...
    # IS 456 Clause 25.1.2: Slender if le/D (or le/b) > 12.0
    short_x = lam_x <= 12.0
    short_y = lam_y <= 12.0
    ...
    # Nominal shear strength of concrete (tau_c, used as base strength)
    tau_c_base = 0.62 * math.sqrt(fck) / 1.0 # 0.62 * sqrt(fck) is an approx for minimum tau_c
...
def main():
    ...
    with c_up:
        ...
        if uploaded_file is not None:
            try:
                ...
                # Ensure bars list of lists is converted back to list of tuples for use
    ...
    with c2:
        ...
        # Load inputs in kN/kNm for user convenience, convert to N/Nmm internally
    ...
    with cL3:
        # Note: Left/Right counts include the corner bar if it's placed by n_top/n_bot
    ...
    with c_check:
        st.metric(f"Slenderness $\\lambda_x = l_{{e,x}}/r_x$", f"{state['lam_x']:.1f} ({'Short' if state['short_x'] else 'Slender'})")
        st.metric(f"Slenderness $\\lambda_y = l_{{e,y}}/r_y$", f"{state['lam_y']:.1f} ({'Short' if state['short_y'] else 'Slender'})")
    ...
    # Custom alpha exponent for P-M interaction (0.8 to 2.0 based on P/Puz)
    ...
    with c4_3: 
        st.metric("Utilization (Σ ≤ 1)", f"**{state['util']:.2f}**")
        if state['util'] <= 1.0: 
            st.success("✅ Biaxial interaction PASS.")
        else: 
            st.error("❌ Biaxial interaction FAIL — **Increase steel area** in Section 2.")
    ...
    with c5_3:
        ...
        if state["tie_spacing"] <= state["s_governing_tie"]: 
            st.success(f"Tie Spacing PASS: {state['tie_spacing']:.0f} mm $\\le$ {state['s_governing_tie']:.0f} mm.")
        else: 
            st.error(f"Tie Spacing FAIL: $\\le$ **{state['s_governing_tie']:.0f} mm**.")
    st.markdown("---")

    # --- 6. Output Summary ---
    st.header("6️⃣ Printable Output Summary")
    As_long = state['As_long']
    As_min = 0.008 * Ag # Based on IS 13920 minimum
    
    # Simple estimate of required area based on utilization
    As_governing_req = max(As_min, As_long * (state.get("util", 1.0) ** (1/state.get("alpha", 1.0))))
