...
# Import Dict for cleaner typing, if needed
...
try:
    # SHAPELY is crucial for KML Polygon geometry
...
def polygon_buffer(xy_coords: List[Tuple[float, float]], offset_dist: float) -> Optional[List[Tuple[float, float]]]:
    ...
    try:
        ...
        # join_style=2 is for mitered join, good for right-angles/corners
...
def polygon_area_perimeter_xy(xy_pts: List[Tuple[float,float]]) -> Tuple[float,float]:
    ...
    pts = xy_pts
    # Ensure the list is closed for calculation
    if pts[0] != pts[-1]:
        pts = xy_pts + [xy_pts[0]]
...
def kml_to_area_perimeter_and_xy(file) -> Tuple[float, float, Optional[List[Tuple[float, float]]]]:
    """Main KML loader function: returns Area, Perimeter, and XY coordinates."""
    file.seek(0)
...
with st.sidebar:
    ...
    if mode == "Length & Width":
        ...
        if kml is not None and SHAPELY_AVAILABLE:
            # FIX: Use function that returns XY coords
...
rows = []
cum_vol = 0.0
# Flag to indicate if we should use polygon (Shapely) logic for geometry
...
if is_polygon_mode:
    ...
    offset_bbl = m_exc * depth_bg # Positive distance in meters
...
while True:
    ...
    if is_polygon_mode:
        ...
        offset_abl = m_in * lift_h # Positive distance
        ...
        offset_berm = bench_w # Positive distance
...
def plotly_3d_full_stack(df: pd.DataFrame, avg_ground_rl: float, is_polygon_plot: bool):
        ...
        if is_polygon_plot:
            ...
            if v1_count == v2_count:
                ...
                v_count += v1_count + v2_count
                 ...
                 v_count += v1_count + v2_count
                 continue # Skip face generation if vertex counts mismatch
    ...
    if xy_bl and SHAPELY_AVAILABLE:
        ...
        # Fallback to rectangular bounds if KML/Shapely failed
...
def generate_section_profile(df: pd.DataFrame, depth_bg: float, m_exc: float, axis: str = "W") -> dict:
    ...
    # FIX: Include 'Volume (Cum)' to allow checking if the row is a Berm row.
    ...
    for i in range(len(df_rect_metrics)):
        ...
        if row['Level'].startswith('ABL') and row['Volume (Cum)'] is not None:
            # For ABL Berm rows (where Volume is recorded)
...
# 2. 3D Plot
# Pass the is_polygon_mode flag to the 3D function
...
st.caption(f"Notes: The 3D model now displays the **actual KML polygon shape** (if **Shapely** is installed and KML uploaded). The 2D cross-section is plotted using the equivalent rectangular Length/Width.")
