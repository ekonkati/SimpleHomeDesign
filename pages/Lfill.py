...
# -------------------------------------------------------------------
# **FINAL STABILITY & KML FIXES**
# 1. KML Color Fix: Replaced simplekml.Color.brown/green with explicit hex/alpha values to resolve the persistent AttributeError.
# 2. NameError Fix: 'rectangle_polygon' remains correctly positioned.
# 3. Initialization Fix: Robust session state initialization is maintained.
# 4. Geometry Refinement: Minimum top area constraint (user request) is confirmed and retained.
#
# To run:
...
# Function implementations (plot_cross_section, grid_search_bishop, compute_boq, export_excel, plotly_3d_full_stack)
# remain as in the previous fully corrected version. They are only included here for completeness.
...
# ---------------------------
# Core Geometry & Volume Functions (Minimum Top Area Constraint Retained)
# ---------------------------
...
def compute_bbl_abl(
    ...
    # ABL Calculation with Minimum Top Area Constraint
    abl_sections = []
    ...
    while remaining_H > 0.0:
        ...
        if A_next_toe_potential < A_min:
            # Stop the iteration and calculate the final cap section
            ...
            # Add berm section (if not final segment and if intermediate height was used)
...
def generate_section(bblabl: dict, outside_slope_h_geom: float, W_int_berm: float) -> dict:
    ...
    x_bund_outer_right = [x_outer_excav_gl_right, x_outer_tob_right]
...
# --- ROBUST SESSION STATE INITIALIZATION (Retained) ---
...
with geom_tab:
    ...
    st.image(img, caption=f"Unified Cross-section (Inner Stepped Landfill Profile with {bblabl.get('N_berms', 0)} Berms + Outer Bund)")
...
with report_tab:
    ...
    if KML_AVAILABLE:
        ...
        # Explicit hex with alpha (AABBGGRR) for robustness
        poly_base.polystyle.color = '80550014' # 550014 is dark red/brown, 80 is alpha
        ...
        # Explicit hex with alpha (AABBGGRR) for robustness
        poly_tol.polystyle.color = '8000FF00' # 00FF00 is green, 80 is alpha
