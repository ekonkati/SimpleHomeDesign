...
# 1. Initialization: Reverted 'geom' back to a dataclass but used a robust
#    initialization check (if 'geom' is not an instance of GeometryInputs)
#    to prevent initialization errors on Streamlit rerun.
# 2. Type/Attribute Error: Confirmed all st.number_input calls correctly
#    access the GeometryInputs object attributes.
#
# To run:
...
# --- Default Initialization Values (for the dataclasses) ---
...
# ---------------------------
# Core Geometry & Volume Functions (Crucial to Stepped Berm Fix)
# ---------------------------
...
def compute_bbl_abl(
    ...
    # (Function logic for BBL/ABL volume calculation - identical to the last working version)
...
def generate_section(bblabl: dict, outside_slope_h_geom: float, W_int_berm: float) -> dict:
    # (Function logic for cross-section generation - identical to the last working version with berm fix)
    ...
    # Ensure the very top is included
    ...
    x_outer_bund_gl_right = W_GL / 2.0
...
# (Utility functions: plot_cross_section, grid_search_bishop, compute_boq, export_excel, plotly_3d_full_stack - all restored)
...
# --- ROBUST SESSION STATE INITIALIZATION (FINAL ATTEMPT) ---
...
if "geom" not in st.session_state or not isinstance(st.session_state.geom, GeometryInputs): 
    st.session_state.geom = DEFAULT_GEOM # Back to dataclass for cleaner attribute access
...
# ---------------------------
# 2) Geometry (BBL/ABL inputs) - Fixed to use geom attributes
# ---------------------------
with geom_tab:
    st.markdown("### BBL/ABL Parameters")
    c1, c2, c3 = st.columns(3)

    geom_init = st.session_state.geom # Safely initialized GeometryInputs object

    with c1:
        # **FIXED:** Accessing dataclass attributes directly now works after robust init
    ...
    # Re-save the GeometryInputs object
    ...
    # (Dimensions and Lifespan metrics - Omitted for brevity)
